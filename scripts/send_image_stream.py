...
The `stream` bitstream ties axis_cam_bridge's gray_input high, because the
whole pipeline behind it is luminance-only - so --mode gray is the one that
matches the hardware. --mode rgb565 is kept for a bridge built with
gray_input low, and will come out as noise otherwise.
...
def main():
    ...
    parser.add_argument("--mode", choices=["gray", "rgb565"], default="gray", help="Image mode; the stream bitstream expects gray (default: gray)")
